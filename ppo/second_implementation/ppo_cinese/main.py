...
from gymnasium.wrappers import ResizeObservation, FlattenObservation, GrayscaleObservation
...
parser.add_argument('--initial_entropy_coef', type=float, default=0.05, help='Entropy coefficient of Actor')
parser.add_argument('--entropy_coef_decay', type=float, default=25000, help='Decay rate of entropy_coef')
...
def main():
    ...
    if opt.Loadmodel:
        ...
        # Converte i vecchi dati di train_rewards (float) nel nuovo formato (tupla)
        if train_rewards and isinstance(train_rewards[0], float):
            print("Converting old train_rewards format...")
            # Stima i passi per i vecchi dati basandosi sull'intervallo di salvataggio
            # NOTA: Questa è un'approssimazione per la visualizzazione
            estimated_steps_per_episode = agent.total_steps_taken / len(train_rewards) if len(train_rewards) > 0 else 0
            train_rewards = [(int(i * estimated_steps_per_episode), reward) for i, reward in enumerate(train_rewards)]
    ...
    if opt.render:
        ...
        while total_steps < opt.Max_train_steps:
            ...
            while not done:
                ...
                if done:
                    train_rewards.append((total_steps, episode_reward))
                ...
                if total_steps % opt.eval_interval == 0:
                    ...
                    print(f'  Episode: {total_steps // opt.T_horizon}',
                          f'  Train Rewards: {train_rewards[-1][1]:.2f}',
